...
module.exports = (configValues, utils) => {
  ...
  const { getCityToPLZ, writeValueToJSON } = utils;
  ...
  router.post('/plz', async (req, res) => {
    ...
    // Zu gesetzter Postleitzahl die Stadt bestimmen und festlegen
  ...
  router.post('/geo-coords', async (req, res) => {
    ...
    // Fehler, wenn keine Geo-Koordinaten mitgeschickt wurden
    if (!lat || !lon) {
    ...
    //Fehler, wenn Geo-Koordinaten keine Zahlen sind
